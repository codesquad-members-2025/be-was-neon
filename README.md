...
- 템플릿 엔진처럼 동적으로 처리할 부분을 생성하여 응답에 바꿔넣는다.

### 7단계
- 사용자가 로그인을 한 경우 글쓰기 버튼을 누르면 write.html을 보여준다.
- 로그인하지 않은 경우 로그인페이지로 리다이렉트 된다.
- 작성한 글, 내용, 작성자를 index.html에서 보여준다.
- 다음글, 이전글을 눌러서 다른 글들를 볼수 있다.
- H2, jdbc로 데이터를 저장한다.
- EC2로 배포한다.
...
- 동적으로 html을 생성하던 중 html이 전부 아스키 코드로 출력되는 문제가 있었습니다. 원인은 파일을 읽은 바이트 배열을 String으로 변환하기 위해 Arrays.toString()을 사용했었는데 이 방식은 바이트 배열에 저장된
 아스키코드를 String으로 바꿀뿐이었기 때문에 발생한 문제였습니다. 이후 new String(byte[], StandardCharsets.UTF_8)을 이용하여 문자열로 변환하여 해결했습니다
  다만 여기서 조금 고민이 생겼습니다. new String()을 사용할 경우, 매번 Heap에 새로운 문자열 객체가 생성되기 때문에, 만약 많은 요청이 동시에 들어온다면 성능에 영향을 줄 수 있지 않을까 하는 걱정이 들었습니다.
 그래서 intern()을 사용하면 문자열이 JVM의 상수 풀(String Constant Pool)에 올라가 캐싱되는 효과가 생기니까 더 효율적일 수도 있겠다는 생각이 들어서 관련 내용을 공부해보았습니다. 하지만 공부해본 결과 intern()은 적절하지 않다고 생각이 들었습니다.
  하지만 결론적으로는 intern()을 남용하는 것은 오히려 문제가 될 수 있다고 판단했습니다. 특히 HTML 같은 정적 파일들은 크기가 작지 않을 수 있어서, 이를 모두 상수 풀에 올리는 건 메모리 부담이나 GC 측면에서 오히려 비효율적일 것 같았습니다.
 차라리 정적 파일의 경우에는 문자열 대신 바이트 배열(byte[]) 자체를 캐싱하거나, HTTP 응답에 Cache-Control 헤더를 활용하는 방법이 더 낫다는 결론에 도달했습니다.

- 게시글을 조회 하기 위해 Path Variable을 처리하는 것에 어려움이 있었습니다. 정규식을 이용해서 핸들러를 찾아오긴 했지만 Path Variable을 전달할 방법이 없었습니다.
 핸들러와 PathVariable을 가지는 레코드 객체를 만들고 Handler 인터페이스를 오버라이딩 해서 PathVariable을 주입하고 실제 핸들러에게 작업을 위임시켜 해결했습니다.

- 게시글을 동적으로 처리하다 보니 Renderer에서 DB를 조회하게 되었습니다. 이를 해결하기 위해 외부에서 조회한 글들을 생성자로 주입 받으려 했지만 
 현재 구조에서는 MethodResolver에서 Render를 주입해 주고 있었기 때문에 Renderer에서 DB를 사용하지 않기 위해 MethodResolver에서 DB를 조회하는 것은 맞지 않다고 생각했습니다.
 기존의 TemplateHandler가 아닌 MainPageHandler를 새로 만들어 DB조회를 하고 Renderer를 생성해 값을 주입하는 방식을 사용해서 해결했습니다.