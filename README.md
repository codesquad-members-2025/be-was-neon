...
- 메인 페이지에서 게시글의 이미지를 보여주는 기능을 추가한다.
- 파일 저장시 이미지를 서버에 저장하고 해당 이미지 이름을 포함하는 게시글 객체를 DB에 저장
- 클래스와 퍼블릭 메소드에 javadoc 형식의 주석을 작성하고 작성한 주석을 기반으로 javadoc을 생성한다.
...
- 이미지를 저장하기 위해 Content-Length만큼 데이터를 읽어 저장하는 방식을 사용했지만 UTF-8로 읽다보니 이미지가 제대로 변환되지 않는 문제가 발생했습니다. 해결하기 위해 ISO-8859-1을 사용하니
 이미지는 잘 저장이 되지만 한글로 작성한 게시글 내용이 깨지는 것이 문제가 되었습니다. 게시글 내용의 경우 ISO-8859-1로 인코딩된 문자를 바이트로 변환하고 다시 UTF-8로 변환하는 식으로 해결했으나
 이 과정에서 완전히 안전하게 변환이 된다고 장담할 수 없기 때문에 다른 방법을 더 찾아봐야 할 것 같습니다.

- 게시글 조회시 이미지를 불러오지 못하는 문제가 있었습니다. 이미지를 한글로 저장하는 경우가 있었기 때문에 requestLine으로 요청되는 값이 인코딩되어 해당 파일을 찾지 못하는 것이 원잉이었습니다.
 이미지 이름을 UUID로 바꿔서 저장하여 한글로 인한 문제가 안 일어나게 했고 파일 이름이 중복되는 경우도 예방하도록 했습니다.